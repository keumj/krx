...
_APP_TITLE = "Stock Analysis Lab | KRX"
...
def _load_krx_components_full() -> tuple[pd.DataFrame, str]:
    for path in _krx_components_candidates():
    ...
    raise FileNotFoundError("KRX components CSV not found.")
...
def _drawdown_comparison_chart(
    ...
    market_curve = _drawdown_curve_pct(market_daily, 252).rename("KRX Cap-Weighted")
    chart_df = pd.concat([ticker_curve, sector_curve, market_curve], axis=1).dropna(how="all")

    fig, ax = plt.subplots(figsize=(6.8, 3.8))
    if not chart_df.empty:
        for name, color in [(ticker, "#0f4c81"), ("Sector Cap-Weighted", "#2e7d32"), ("KRX Cap-Weighted", "#b26a00")]:
...
def _rolling_volatility_chart(
    ...
    market_curve = _rolling_volatility_pct(market_daily, 20, 252).rename("KRX Cap-Weighted")
    chart_df = pd.concat([ticker_curve, sector_curve, market_curve], axis=1).dropna(how="all")
...
def _recent_risk_shock_table(
    ...
    recent = pd.concat(
        [
            ...
            _clean_return_series(market_daily).rename("KRX Cap-Weighted Return"),
        ],
        axis=1,
    ...
    if recent.empty:
        return pd.DataFrame(
            columns=[
                ...
                "KRX Cap-Weighted Return",
                "Vs Sector",
                "Vs KRX",
            ]
        )
    recent["Vs Sector"] = recent[f"{ticker} Daily Return"] - recent["Sector Cap-Weighted Return"]
    recent["Vs KRX"] = recent[f"{ticker} Daily Return"] - recent["KRX Cap-Weighted Return"]
    ...
    *,
    ...
    if ticker_vol_252d is not None and sector_vol_252d is not None and market_vol_252d is not None:
        if ticker_vol_252d > sector_vol_252d * 1.15 and ticker_vol_252d > market_vol_252d * 1.15:
            parts.append(
                f"{ticker}는 현재 1년 실현변동성 기준으로 {sector} 섹터와 시가총액가중 KRX보다 변동성이 높은 편입니다."
            )
            risk_bias = "high"
        elif ticker_vol_252d < sector_vol_252d * 0.9 and ticker_vol_252d < market_vol_252d * 0.9:
            parts.append(
                f"{ticker}는 현재 1년 실현변동성 기준으로 {sector} 섹터와 시가총액가중 KRX보다 방어적인 움직임을 보이고 있습니다."
            )
            risk_bias = "low"
        else:
            parts.append(
                f"{ticker}의 현재 리스크 강도는 {sector} 섹터와 시가총액가중 KRX에 대체로 근접한 수준입니다."
            )
    ...
    if beta_market_1y is not None:
        if beta_market_1y >= 1.2:
            parts.append("KRX 대비 베타가 1보다 높아 시장이 흔들릴 때 주가 변동이 더 크게 확대될 가능성이 있습니다.")
            if risk_bias != "low":
                risk_bias = "high"
        elif beta_market_1y <= 0.8:
            parts.append("KRX 대비 베타가 낮아 최근에는 시장 전체보다 민감도가 낮은 흐름으로 해석할 수 있습니다.")
...
def _run_decision_once(
    ...
    *,
    ...
    if not ticker:
        raise ValueError("Provide an KRX ticker for decision analysis.")
    ...
    for label, weight in comparisons:
        ...
        if ticker_return > sector_return and ticker_return > market_return:
            ...
            bullish.append(f"{label} 수익률이 섹터와 KRX을 모두 상회해 상대강도는 우호적입니다. 종목 {_format_pct(ticker_return)}, 섹터 {_format_pct(sector_return)}, KRX {_format_pct(market_return)}입니다.")
            relative_details.append(f"{label}_outperform")
        elif ticker_return < sector_return and ticker_return < market_return:
            relative_score -= weight
            bearish.append(f"{label} 수익률이 섹터와 KRX을 모두 밑돌아 상대강도는 약합니다. 종목 {_format_pct(ticker_return)}, 섹터 {_format_pct(sector_return)}, KRX {_format_pct(market_return)}입니다.")
    ...
    if returns_ctx.market_rank_ytd is not None and returns_ctx.market_count > 0:
        ...
        if market_pct <= 0.10:
            relative_score += 0.3
            bullish.append(f"KRX 전체에서도 YTD 상위권({returns_ctx.market_rank_ytd}/{returns_ctx.market_count})이라 시장 주도주 성격이 있습니다.")
            relative_details.append("market_top_decile")
        elif market_pct >= 0.90:
            relative_score -= 0.3
            bearish.append(f"KRX 전체에서도 YTD 하위권({returns_ctx.market_rank_ytd}/{returns_ctx.market_count})이라 약세 흐름이 뚜렷합니다.")
    ...
    if risk_ctx.beta_market_1y is not None:
        if risk_ctx.beta_market_1y >= 1.2:
            risk_score -= 0.4
            bearish.append(f"KRX 대비 베타가 {risk_ctx.beta_market_1y:,.2f}로 높아 시장 급락 시 충격이 더 커질 수 있습니다.")
            risk_details.append("high_beta")
        elif risk_ctx.beta_market_1y <= 0.8:
            risk_score += 0.4
            bullish.append(f"KRX 대비 베타가 {risk_ctx.beta_market_1y:,.2f}로 낮아 포지션 관리가 상대적으로 수월한 편입니다.")
    ...
    signal_table = pd.DataFrame(
        [
            ...
            {"Metric": "KRX YTD Rank", "Value": "-" if returns_ctx.market_rank_ytd is None else f"{returns_ctx.market_rank_ytd:,d} / {returns_ctx.market_count:,d}"},
            ...
            {"Metric": "Beta vs KRX", "Value": f"{risk_ctx.beta_market_1y:,.2f}" if risk_ctx.beta_market_1y is not None else "-"},
        ]
    )
...
def _relative_returns_chart(
    ...
    market_curve = _base100_index_from_daily_returns(market_daily).rename("KRX Cap-Weighted")
    chart_df = pd.concat([ticker_curve, sector_curve, market_curve], axis=1).dropna(how="all")
...
def _run_returns_once(form: dict[str, str]) -> _ReturnsContext:
    ...
    if not raw_ticker:
        raise ValueError("Provide an KRX ticker for return analysis.")

    components, components_source = _load_krx_components_full()
    ...
    if raw_ticker not in set(components["Symbol"]):
        raise ValueError(f"{raw_ticker} is not available in the shared KRX universe.")
    ...
    prices, price_source = fetch_krx_close_prices(all_symbols, start_date=start_date)
    ...
    if not market_weight_symbols:
        raise ValueError("Shared market cap data for the KRX universe is unavailable.")
        ...
        summary_rows.append(
            {
                ...
                "KRX Cap-Weighted": _format_pct(market_value),
                "Vs Sector": _format_pct(None if ticker_value is None or sector_value is None else ticker_value - sector_value),
                "Vs KRX": _format_pct(None if ticker_value is None or market_value is None else ticker_value - market_value),
            }
        )
...
def _run_risk_once(form: dict[str, str]) -> _RiskContext:
    raw_ticker = str(form.get("ticker", "")).strip().upper()
    if not raw_ticker:
        raise ValueError("Provide an KRX ticker for risk analysis.")
    ...
    summary_table = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "KRX Cap-Weighted": _format_pct(float(market_worst_day_1y.min()) if not market_worst_day_1y.empty else None),
            },
            {
...
def _factor_beta_chart(ticker: str, beta_market_curve: pd.Series, beta_sector_curve: pd.Series) -> str:
    chart_df = pd.concat(
        [
            beta_market_curve.rename("Beta vs KRX"),
    ...
    fig, ax = plt.subplots(figsize=(6.8, 3.8))
    if not chart_df.empty:
        if "Beta vs KRX" in chart_df.columns:
            ax.plot(chart_df.index, chart_df["Beta vs KRX"], color="#0f4c81", linewidth=2.0, label="Beta vs KRX")
...
def _factor_residual_chart(ticker: str, residual_market_daily: pd.Series, residual_sector_daily: pd.Series) -> str:
    chart_df = pd.concat(
        [
            _cumulative_residual_pct(residual_market_daily).rename("Residual vs KRX"),
    ...
    fig, ax = plt.subplots(figsize=(6.8, 3.8))
    if not chart_df.empty:
        if "Residual vs KRX" in chart_df.columns:
            ax.plot(chart_df.index, chart_df["Residual vs KRX"], color="#0f4c81", linewidth=2.0, label="Residual vs KRX")
    ...
    *,
    ...
    if beta_market_60d is not None:
        if beta_market_60d >= 1.2:
            ...
            parts.append(f"{ticker}의 최근 60거래일 베타(beta)는 시장 평균에 가까워, KRX과 비교적 비슷한 민감도로 움직였습니다.")
...
def _run_factor_once(form: dict[str, str]) -> _FactorContext:
    raw_ticker = str(form.get("ticker", "")).strip().upper()
    if not raw_ticker:
        raise ValueError("Provide an KRX ticker for factor and regime analysis.")
    ...
    if beta_market_60d is not None and not market_joined.empty:
        residual_market_daily = (market_joined["ticker"] - (beta_market_60d * market_joined["market"])).rename("Residual vs KRX")
    ...
    recent_factor_table = pd.concat(
        [
            ...
            _clean_return_series(market_daily).rename("KRX Return"),
            residual_market_daily.rename("Residual vs KRX"),
            ...
            market_member_count.rename("KRX Members Used"),
        ],
        axis=1,
    ...
    if recent_factor_table.empty:
        ...
        for col in ["Sector Members Used", "KRX Members Used"]:
            if col in recent_factor_table.columns:
...
<head>
  ...
  <title>Stock Analysis Lab | KRX - 주가 예측</title>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Stock Analysis Lab | KRX - 워크포워드 검증</title>
  <style>{_base_css()}</style>
</head>
<body>"""
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Stock Analysis Lab | KRX - 재무제표·밸류에이션</title>
  <style>{_base_css()}</style>
</head>
<body>"""
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Stock Analysis Lab | KRX - 기술적 분석</title>
  <style>{_base_css()}</style>
</head>
<body>"""
...
def _html_returns_page(
    ...
    *,
    ...
    if ctx is None and not error:
        info_html += (
            "<div class=\"notice ok\">"
            "수익률 비교 페이지는 공유 KRX SQLite/캐시 데이터를 바탕으로 "
    ...
    if ctx is not None:
        ...
        <div class="metrics">
          ...
          <div class="metric"><span>KRX YTD Rank</span><strong>{html.escape(market_rank_text)}</strong></div>
        </div>
        """
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Stock Analysis Lab | KRX - 수익률 비교</title>
  <style>{_base_css()}</style>
</head>
<body>"""
  ...
  <div class="wrap">
    {_page_head(_APP_TITLE)}
    <div class="sub">공유 KRX 데이터로 종목 수익률과 섹터·지수 상대강도를 비교하는 페이지</div>
...
def _html_risk_page(
    ...
    *,
    ...
    if ctx is None and not error:
        info_html += (
            "<div class=\"notice ok\">"
            "리스크 대시보드는 공유 KRX SQLite 데이터를 이용해 선택 종목의 "
    ...
    if ctx is not None:
        ...
        <div class="metrics">
          ...
          <div class="metric"><span>Beta vs KRX</span><strong>{_format_metric(ctx.beta_market_1y, 2) if ctx.beta_market_1y is not None else "-"}</strong></div>
          ...
          <div class="metric"><span>KRX 1Y Vol Rank</span><strong>{html.escape(market_rank_text)}</strong></div>
        </div>
        """
        commentary_html = f"""
        <div class="card">
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Stock Analysis Lab | KRX - 리스크 대시보드</title>
  <style>{_base_css()}</style>
</head>
<body>"""
  ...
  <div class="wrap">
    {_page_head(_APP_TITLE)}
    <div class="sub">공유 KRX 데이터로 변동성, 낙폭, 베타를 점검하는 리스크 페이지</div>
...
def _html_factor_page(
    ...
    *,
    ...
    if ctx is None and not error:
        info_html += (
            "<div class=\"notice ok\">"
            "팩터·레짐 랩은 선택 종목을 시장(KRX)과 섹터 요인으로 분해해, "
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Stock Analysis Lab | KRX - 팩터·레짐 랩</title>
  <style>{_base_css()}</style>
</head>
<body>"""
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Stock Analysis Lab | KRX - 의사결정 대시보드</title>
  <style>{_base_css()}</style>
</head>
<body>"""
...
def _krx_sqlite_max_date(sqlite_path: Path) -> str | None:
...
def _collect_post_refresh_items(root_dir: Path) -> list[dict[str, object]]:
    ...
    shared_db_root = data_dir / "krx_shared_db"
    krx_sqlite_path = shared_db_root / "krx_shared_prices.sqlite"

    items: list[dict[str, object]] = []
    metrics_csv = data_dir / "krx_all_metrics_prices.csv"
    mcap_csv = data_dir / "krx_market_caps.csv"
    items.extend(
        [
            {
                "dataset": "krx_all_metrics_prices",
            ...
            {
                "dataset": "krx_shared_prices_sqlite",
                "latest_date": _krx_sqlite_max_date(krx_sqlite_path),
                "rows": 0,
                "source": "refresh_stock_data.bat",
                "path": str(krx_sqlite_path.resolve()),
            },
        ]
    )
    return items
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th>Started</th>
              <th>Finished</th>
              <th>KRX Old Max Date</th>
              <th>KRX New Max Date</th>
              <th>Financial SQLite Added</th>
              <th>KRX SQLite Added</th>
              <th>KRX MCap Updates</th>
              <th>Error</th>
            </tr>
          </thead>
      ...
      try {{
        ...
        const runRows = runs.map((run) => `
          <tr>
            ...
            <td>${{esc(run.krx_old_max_date)}}</td>
            <td>${{esc(run.krx_new_max_date)}}</td>
            <td>${{esc(run.financial_sqlite_rows_added)}}</td>
            <td>${{esc(run.krx_sqlite_added_rows)}}</td>
            <td>${{esc(run.krx_sqlite_market_cap_updates)}}</td>
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _handle_returns_run(self) -> None:
            ...
            if ticker_note_error and not effective_ticker:
                ...
                self.__class__.state_ret_error = ticker_note or "Provide an KRX ticker for return analysis."
                self._send_html(
                    _html_returns_page(
                        self.state_ret_form,
                        ctx=self.state_ret_ctx,
        ...
        def _handle_risk_run(self) -> None:
            ...
            if ticker_note_error and not effective_ticker:
                ...
                self.__class__.state_risk_error = ticker_note or "Provide an KRX ticker for risk analysis."
                self._send_html(
                    _html_risk_page(
                        self.state_risk_form,
                        ctx=self.state_risk_ctx,
        ...
        def _handle_factor_run(self) -> None:
            ...
            if ticker_note_error and not effective_ticker:
                ...
                self.__class__.state_factor_error = ticker_note or "Provide an KRX ticker for factor and regime analysis."
                self._send_html(
                    _html_factor_page(
                        self.state_factor_form,
                        ctx=self.state_factor_ctx,
        ...
        def _handle_decision_run(self) -> None:
            ...
            if ticker_note_error and not effective_ticker:
                self.__class__.state_dec_ctx = None
                self.__class__.state_dec_error = ticker_note or "Provide an KRX ticker for decision analysis."
                self._send_html(
                    _html_decision_page(
                        self.state_dec_form,
                        ctx=self.state_dec_ctx,
        ...
        def _refresh_status_payload(cls) -> dict[str, object]:
            ...
            metrics_csv = root_dir / "data" / "krx_all_metrics_prices.csv"
            latest_date = _latest_date_from_csv(metrics_csv)
        ...
        def _run_refresh_job(cls, run_id: int) -> None:
            ...
            krx_rows_added = 0
            krx_market_cap_updates = 0
            root_dir = _project_root_dir()
            refresh_cmd, refresh_label = _refresh_subprocess_command(root_dir)
            krx_db_path = root_dir / "data" / "krx_shared_db" / "krx_shared_prices.sqlite"
            krx_old_max_date: str | None = _krx_sqlite_max_date(krx_db_path)
            krx_new_max_date: str | None = krx_old_max_date
            ...
            if not refresh_cmd:
                ...
                try:
                    ...
                    if proc.stdout is not None:
                        for line in proc.stdout:
                            ...
                            m_krx = re.search(r"SQLite added rows=(\d+), market_cap_updates=(\d+)", stripped)
                            if m_krx:
                                krx_rows_added = int(m_krx.group(1))
                                krx_market_cap_updates = int(m_krx.group(2))

                            m_dates = re.search(r"SQLite date range: old_max=([^,]+), new_max=(.+)$", stripped)
                            if m_dates:
                                krx_old_max_date = m_dates.group(1).strip()
                                krx_new_max_date = m_dates.group(2).strip()
                ...
                krx_new_max_date = _krx_sqlite_max_date(krx_db_path) or krx_new_max_date
            ...
            history_row = {
                ...
                "krx_old_max_date": krx_old_max_date,
                "krx_new_max_date": krx_new_max_date,
                "financial_sqlite_rows_added": financial_rows_added,
                "krx_sqlite_added_rows": krx_rows_added,
                "krx_sqlite_market_cap_updates": krx_market_cap_updates,
...
def launch_web_gui(host: str = "localhost", port: int = 8522, open_browser: bool = False) -> None:
...
__all__ = [
    ...
    "launch_web_gui", "launch_stock_forecast_web_gui", "run_web_gui",
]
