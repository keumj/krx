...
def _has_runtime_data(root: Path) -> bool:
    ...
    return (
        ...
        and (data_dir / "krx_all_metrics_prices.csv").is_file()
...
def _run_gui(argv: list[str]) -> int:
    ...
    from pipeline_krx_stock.web_gui import launch_web_gui
    ...
    print(f"Starting Equity Analysis Lab | KRX at http://{host}:{port}", flush=True)
    launch_web_gui(host=host, port=port)
    return 0


def _run_refresh(argv: list[str]) -> int:
    from pipeline_common.refresh_krx_shared_prices import main as refresh_main
    ...
    print(
        "\n".join(
            [
                ...
                "  refresh [refresh options]                Refresh KRX shared data.",
                "",
                "Examples:",
