...
def _ensure_stock_lab_running(port: int = 8522) -> None:
    ...
    try:
        ...
        if getattr(sys, "frozen", False):
            ...
            cmd = [sys.executable, "-m", "pipeline_krx_stock.cli", "--web-gui", "--port", str(port), "--host", "localhost"]
        subprocess.Popen(cmd, cwd=str(root), creationflags=subprocess.CREATE_NEW_CONSOLE if os.name == 'nt' else 0)
    except Exception:
        pass


def _dispatch_stock_lab_external_select(ticker: str, *, target: str = "/page6", port: int = 8522) -> tuple[bool, str]:
...
def _scoring_page(ctx: _PageContext) -> str:
    ...
    <div class="card" style="margin-top: 12px; border-top: 4px solid var(--ok-line);">
      <h2>신규 추천 종목 (미보유 KRX 상위 10)</h2>
      <p class="hint">현재 포트폴리오에 보유하지 않은 KRX 종목 중 통합 스코어가 가장 높은 종목들입니다. 새로운 투자 기회 발굴에 참고하십시오.</p>
...
def _optimization_page(ctx: _PageContext) -> str:
    ...
    <div class="card">
      <h2>포트폴리오 최적화</h2>
      <p class="hint">KRX 복제, 공격형, 방어형 포트폴리오를 현재 보유 종목과 비교하면서 제약조건을 반영해 재구성합니다.</p>
    ...
    <div class="grid-3">
      <div class="card">
        ...
        <p class="hint" style="margin-bottom: 12px;">
          ...
          <strong>방법:</strong> 유니버스 내에서 섹터 비중과 시가총액 가중치를 KRX 지수와 유사하게 매칭하여 재구성합니다.<br>
    ...
    """
    return _layout("Portfolio Lab | 최적화", "포트폴리오를 통한 KRX 복제와 성향별 구성 추천", "optimization", ctx, body)
...
_REFRESH_JOB_DEFS: list[dict[str, str]] = [
    ...
    },
    {
        "job_id": "news",
        "label": "뉴스",
        "description": "KRX 구성 종목 뉴스 기사와 뉴스 분석 대기열을 shared SQLite에 적재합니다.",
        "module": "pipeline_common.refresh_krx_news",
        "button_label": "뉴스 갱신",
    },
]
...
def _collect_stock_refresh_snapshot(root_dir: Path) -> dict[str, object]:
    ...
    metrics_path = data_dir / "krx_all_metrics_prices.csv"
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            try:
                if path == "/redirect_stock_lab":
                    ...
                    _ensure_stock_lab_running(8522)
                    target = f"http://localhost:8522/page6?ticker={ticker}&intent={intent}"
                ...
                if path == "/sync_stock_lab":
                    ...
                    ok, message = _dispatch_stock_lab_external_select(ticker, target="/page6", port=8522)
                    if ok:
