from __future__ import annotations

import argparse

from .web_gui import launch_web_gui


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="KRX data refresh GUI backend")
    parser.add_argument("--web-gui", action="store_true", help="Accepted for shell compatibility")
    parser.add_argument("--host", default="localhost", help="Host for the KRX data refresh GUI")
    parser.add_argument("--port", type=int, default=8517, help="Port for the KRX data refresh GUI")
    parser.add_argument("--open-browser", action="store_true", help="Open browser automatically")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    launch_web_gui(host=str(args.host), port=int(args.port), open_browser=bool(args.open_browser))


if __name__ == "__main__":
    main()
