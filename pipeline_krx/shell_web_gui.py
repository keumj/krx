...
_BACKENDS: tuple[_BackendConfig, ...] = (
    _BackendConfig("krx_refresh", "KRX 데이터갱신", "pipeline_krx.data_refresh_gui", "localhost", 8517),
    _BackendConfig("stock", "종목분석", "pipeline_krx_stock", "localhost", 8522),
    _BackendConfig("stock_news", "뉴스분석", "pipeline_krx_stock_news", "localhost", 8524),
    _BackendConfig("portfolio", "포트폴리오", "pipeline_krx_portfolio", "localhost", 8525),
    _BackendConfig("macro", "매크로", "pipeline_krx_macro", "localhost", 8526),
)

_PAGES: tuple[_PageLink, ...] = (
    _PageLink("krx-refresh", "data_refresh", "KRX 데이터갱신", "/refresh", backend_key="krx_refresh", source_label="KRX 데이터갱신"),
    _PageLink("macro-overview", "macro", "매크로 개요", "/overview", backend_key="macro"),
    _PageLink("macro-regime", "macro", "레짐", "/regime", backend_key="macro"),
    _PageLink("macro-rates", "macro", "금리/커브", "/rates", backend_key="macro"),
    _PageLink("macro-risk", "macro", "위험자산", "/risk", backend_key="macro"),
    _PageLink("macro-dollar", "macro", "달러/원자재", "/dollar", backend_key="macro"),
    _PageLink("macro-playbook", "macro", "섹터 플레이북", "/playbook", backend_key="macro"),
...
_SECTIONS: tuple[_SectionConfig, ...] = (
    _SectionConfig("data_refresh", "KRX 데이터갱신", ("krx-refresh",), ("krx_refresh",)),
    _SectionConfig("macro", "매크로", ("macro-overview", "macro-regime", "macro-rates", "macro-risk", "macro-dollar", "macro-playbook"), ("macro",)),
    ...
    _SectionConfig("stock_news", "뉴스분석", ("news-overview", "news-event", "news-spillover", "news-divergence", "news-reset", "news-volatility", "news-topics"), ("stock_news",)),
    _SectionConfig("stock", "종목분석", ("stock-forecast", "stock-financials", "stock-technical", "stock-returns", "stock-risk", "stock-factor", "stock-decision", "stock-wfv"), ("stock",)),
)
...
<body>
  <div class="app">
    <aside class="sidebar">
      <div class="brand">
        <h1>KRX</h1>
        <p>KRX 데이터갱신, 포트폴리오, 뉴스분석, 종목분석 화면을 한곳에서 보고, 왼쪽 워터폴 목록으로 페이지를 전환합니다.</p>
      </div>
...
def launch_web_gui(host: str = "localhost", port: int = 8518, open_browser: bool = False) -> None:
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            selected_page_id = str(query.get("page", ["krx-refresh"])[0]).strip() or "krx-refresh"
