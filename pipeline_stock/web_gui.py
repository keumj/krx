...
def _nav(active: str, *, enable_technical_page: bool = False) -> str:
    ...
    <div class="nav">
      ...
      <a class="{c2}" href="/page2">재무제표·밸류에이션</a>
      {"<a class=\"" + c3 + "\" href=\"/page3\">기술적 분석</a>" if enable_technical_page else ""}
      {"<a class=\"" + c4 + "\" href=\"/page4\">수익률 비교</a>" if enable_technical_page else ""}
      {"<a class=\"" + c5 + "\" href=\"/page5\">리스크 대시보드</a>" if enable_technical_page else ""}
      {"<a class=\"" + cf + "\" href=\"/factor-regime\">팩터·레짐 랩</a>" if enable_technical_page else ""}
      {"<a class=\"" + c6 + "\" href=\"/page6\">의사결정 대시보드</a>" if enable_technical_page else ""}
      {"<a class=\"" + c8 + "\" href=\"/page8\">워크포워드 검증</a>" if enable_technical_page else ""}
    </div>
    {sync_script}
    """
